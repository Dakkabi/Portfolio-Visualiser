...
from backend.src.core.utils.path_util import get_resources_path
...
def get_db_broker_by_name(db: Session, name: str):
...
def create_db_broker_from_file(db: Session, file_path: str = None):
    ...
    with open(file_path, "r") as f:
        for line in f:
            if get_db_broker_by_name(db=db, name=line.strip()):
                continue
